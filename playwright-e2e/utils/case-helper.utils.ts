...
export class CaseHelperUtils {
   ...
   */
  async createCase(userId, userEmail: string, userPassword: string, caseData: Record<string, any>) {
  ...
  private async getEventToken(userId: string, bearerToken: string, eventId: string): Promise<string> {
    ...
    const url: string = `${urlConfig.ccd_data_api_url}/case-types/A58/event-triggers/citizen-create-application`
  ...
  async triggerEvent(userId, caseId: string, eventId: string, eventData: Record<string, any>) {
    const context = await this.createApiContext();
