...
test.describe('e2e submit journeys', () => {
  ...
  test('submitting application where spouse is not a partner', e2eTestTags, async ({ page }, testInfo) => {
  ...
  test('submitting application with spouse or civil partner', e2eTestTags, async ({ page }, testInfo) => {
    const app = new App(page);
    const appOneFirstName = faker.person.firstName();
    const appOneLastName = faker.person.lastName();
    const appTwoFirstName = faker.person.firstName();
