...
const langAssertions = (language, content) => {
  ...
  const { label, example, yes, no, applicant1AdditionalName, add, another, remove, errors } = content;
...
describe('other names content', () => {
  ...
  it("should display correct content under the 'yes' radio when names array is empty", () => {
    ...
    expect((applicant1AdditionalName?.label as Function)(generatedContent)).toBe(enContent.applicant1AdditionalName);
