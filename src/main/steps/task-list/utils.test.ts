    ...
    test('Should return false if applyingWith is not present', async () => {
      ...
      describe('getPersonalDetailsStatus', () => {
        test.each([
          ...
          expect(getPersonalDetailsStatus({ ...userCase, ...data }, <'applicant1' | 'applicant2'>userType)).toBe(
            expected
          );
        });
      });
      ...
      describe('getContactDetailsStatus', () => {
        test.each([
          ...
          expect(getContactDetailsStatus({ ...userCase, ...data }, <'applicant1' | 'applicant2'>userType)).toBe(
            expected
          );
        });
      });
