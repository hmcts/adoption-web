...
describe('reviewPaySubmitSequence', () => {
  test('should contain 4 entries in review pay and submit screen sequence', () => {
    ...
    expect(reviewPaySubmitSequence[2].getNextStep({})).toBe('/review-pay-submit/payment/pay-your-fee');

    expect(reviewPaySubmitSequence[3].url).toBe('/review-pay-submit/payment/pay-your-fee');
    expect(reviewPaySubmitSequence[3].showInSection).toBe('reviewPaySubmit');
    expect(reviewPaySubmitSequence[3].getNextStep({})).toBe('/review-pay-submit/payment/payment-callback');

    expect(reviewPaySubmitSequence[4].url).toBe('/review-pay-submit/payment/payment-callback');
    expect(reviewPaySubmitSequence[4].showInSection).toBe('reviewPaySubmit');
    expect(reviewPaySubmitSequence[4].getNextStep({})).toBe('/task-list');
  });
});
