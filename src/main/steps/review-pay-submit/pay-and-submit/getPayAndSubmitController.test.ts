...
import GetPayAndSubmitController from './getPayAndSubmitController';

describe('GetPayAndSubmitController', () => {
  const controller = new GetPayAndSubmitController(__dirname + './template', generateContent);
  ...
  it('should throw error when feel lookup api fails', async () => {
    ...
    try {
      ...
      expect(err).toEqual(new Error('GetPayAndSubmitController unable to get fee from fee-register API'));
      expect(mockGetFee).toHaveBeenCalledWith(req.locals.logger);
