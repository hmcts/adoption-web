...
describe('eligibilitySequence', () => {
  test('should contain 7 entries in eligibility screen sequence', () => {
    expect(eligibilitySequence).toHaveLength(7);
    ...
    expect(eligibilitySequence[3].getNextStep({ under21Eligible: YesOrNo.YES })).toBe('/eligibility/domicile');
    expect(eligibilitySequence[3].getNextStep({ under21Eligible: YesOrNo.NO })).toBe('/eligibility/cannot-apply');

    expect(eligibilitySequence[4].url).toBe('/eligibility/domicile');
    expect(eligibilitySequence[4].showInSection).toBe('eligibility');
    expect(eligibilitySequence[4].getNextStep({ domicileEligible: YesOrNo.YES })).toBe('/eligibility/lived-uk');
    expect(eligibilitySequence[4].getNextStep({ domicileEligible: YesOrNo.NO })).toBe('/eligibility/cannot-apply');

    expect(eligibilitySequence[5].url).toBe('/eligibility/lived-uk');
    expect(eligibilitySequence[5].showInSection).toBe('eligibility');
    expect(eligibilitySequence[5].getNextStep({ livedUKEligible: YesOrNo.YES })).toBe('/login');
    expect(eligibilitySequence[5].getNextStep({ livedUKEligible: YesOrNo.NO })).toBe('/eligibility/cannot-apply');

    expect(eligibilitySequence[6].url).toBe('/eligibility/cannot-apply');
    expect(eligibilitySequence[6].showInSection).toBe('eligibility');
    expect(eligibilitySequence[6].getNextStep({})).toBe('/login');
  });
});
