...
describe('children > NationalityPostController', () => {
  let req;
  let res;
  let controller;
  ...
  describe('when there are no form errors', () => {
    ...
    describe('and when addButton is pressed', () => {
      ...
      describe('and when addAnotherNationality is present in formData', () => {
        ...
        test('should add the country in userCase childrenAdditionalNationalities session data', async () => {
          await controller.post(req, res);
          expect(req.session.errors).toEqual([]);
          expect(req.session.userCase.childrenAdditionalNationalities).toEqual(['MOCK_COUNTRY']);
    ...
    describe('and when addButton is not pressed', () => {
      ...
      test('should add the country in userCase childrenAdditionalNationalities session data', async () => {
        await controller.post(req, res);
        expect(req.session.errors).toEqual([]);
        expect(req.session.userCase.childrenAdditionalNationalities).toEqual([]);
        expect(req.session.save).toHaveBeenCalled();
        expect(mockGetNextStepUrl).toHaveBeenCalledWith(req, { childrenAdditionalNationalities: [] });
      });
    });
  });
  ...
  describe('when this.fields is a function', () => {
    ...
    test('should set the formData fields in userCase childrenAdditionalNationalities session data', async () => {
      await controller.post(req, res);
      expect(req.session.errors).toEqual([]);
      expect(req.session.userCase.childrenAdditionalNationalities).toEqual(['MOCK_COUNTRY']);
      expect(req.session.save).toHaveBeenCalled();
