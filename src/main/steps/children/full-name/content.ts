...
export const form: FormContent = {
  fields: {
    childrenFirstName: {
      ...
      labelSize: null,
      parser: data => (data as Record<string, string>).childrenFirstName?.replace(/\s{2,}/g, ' ').trim(),
    ...
    childrenLastName: {
      ...
      parser: data => (data as Record<string, string>).childrenLastName?.replace(/\s{2,}/g, ' ').trim(),
      //childrenLastName.replace(/\s{2,}/g,' '),
      validator: isChildrenNameValid,
    },
  },
  submit: {
