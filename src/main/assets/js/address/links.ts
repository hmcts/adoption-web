...
const toggleUkAddressFields = (toggle: string): void => {
  ...
  //TODO remove below 2 lines if we need international address fields in future
  (qs('.govuk-form-group.address3') as HTMLElement).classList['add'](hidden);
  (qs('.govuk-form-group.addressCountry') as HTMLElement).classList['add'](hidden);
};
